...
def conv1x1(in_c, out_c):
    # 1x1 convolution preserves spatial resolution while remapping channels from `in_c` to `out_c`.
...
class TemporalSE(nn.Module):
    ...
    def forward(self, x):
        ...
        y = y.permute(0,2,1).view(B,T,C,1,1)  # Broadcast weights across spatial grid per frame.
        return x * y
...
class TemporalLinearAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        C_h = C // self.heads
        q = q.view(-1, self.heads, C_h, q.shape[-1]) * self.scale  # [BHW,heads,C_h,T]
        k = k.view(-1, self.heads, C_h, k.shape[-1])               # [BHW,heads,C_h,T]
        v = v.view(-1, self.heads, C_h, v.shape[-1])               # [BHW,heads,C_h,T]
        ...
        out = out.permute(0,2,1).contiguous().view(B,H,W,T,C)       # Restore spatial lattice.
...
class CMGCA(nn.Module):
    ...
    def forward(self, A, B):
        g = self.gate(torch.cat([A,B], dim=1))  # [B,C,1,1] gate weights shared across spatial map.
...
class MILHead(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.encoder(x)
        x = self.norm(x[:,0])           # [B,C] pooled from the CLS token.
        return self.head(x)
