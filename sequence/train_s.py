...
# Use dataset variant with seq_source support
...
# ---------------- Sequential-frame input adapter: arbitrary channels -> 3 channels ----------------
...
def main():
    ...
    parser.add_argument('--temporal', type=int, default=1, help='0: single frame; 1: temporal clip')

    # Frame source: this build fixes exp1_vibrant (pre+C1..C8)
    ...
    # Inference-time augmentation (TTA) and SWA
    ...
    # Force exp1_vibrant with exactly nine frames
    seq_source = 'exp1_vibrant'
    if args.T != 9:
        print(f'[INFO] exp1_vibrant -> enforcing 9 frames; overriding --T {args.T} -> 9')
        args.T = 9

    # Dataset and loaders (training split enables temporal-consistent augmentation inside the dataset)
    ...
    # WeightedRandomSampler: sample inversely proportional to class frequency
    ...
    # Class weights (used for CE; optional for Focal)
    ...
    # Print one sample shape to confirm T
    ...
    # Loss function (default CE+LS for stability; switch with --use_focal 1)
    if args.use_focal == 1:
    ...
    # Stage schedule (freeze ConvNeXt, train Swin + heads) with reduced LR and extended fine-tuning
    stage_plan = [
    ...
    for stage in stage_plan:
        ...
        for e in range(stage_epochs):
            ...
            # Control MixUp/CutMix strength per stage
            ...
            # Validation plus threshold sweep
    ...
    # If SWA is enabled: refresh BN stats and switch to SWA model for final evaluation/save
    eval_model = ema.ema if ema is not None else model
    ...
    # Load best threshold from JSON (for final test/deployment)
    ...
    # Final test (EMA/SWA model; threshold from validation best_thr)
