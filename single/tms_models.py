...
def conv1x1(in_c, out_c):
    # 1x1 convolution keeps spatial size while remapping channels.
...
class TemporalSE(nn.Module):
    ...
    def forward(self, x):
        B,T,C,H,W = x.shape
        # Pool over spatial dims to summarise each frame: [B, T, C, H, W] -> [B, T, C].
...
class TemporalLinearAttn(nn.Module):
    ...
    def forward(self, x):
        B,T,C,H,W = x.shape
        # Collapse spatial grid so temporal attention runs per spatial location: [B,T,C,H,W] -> [B*H*W, C, T].
        ...
        # Restore spatial lattice before permuting back to [B, T, C, H, W].
...
class MILHead(nn.Module):
    ...
    def forward(self, x):
        B,T,C,H,W = x.shape
        # Aggregate each frame to a token by averaging spatial dimensions.
...
class TMS_CMAN(nn.Module):
    ...
    def forward(self, x):
        ...
        # Time-stack feature maps so both streams become [B, T, 512, Hs, Ws].
        ...
        # MIL head consumes the temporal stack and outputs class logits [B, num_classes].
        logits = self.mil(out)
        return logits