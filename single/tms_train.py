...
# from datasets_tms import TMSImageFolder, build_transforms
from datasets_tms import TMSSequenceFolder as TMSImageFolder, build_transforms
from tms_models import TMS_CMAN

import pandas as pd

# ---------------- Single-frame input adapter: arbitrary channels -> 3 channels ----------------
class InputAdapter(nn.Module):
    ...
    def forward(self, x):
        # Accepts `[B, in_channels, H, W]` (e.g., grayscale) and returns `[B, out_channels, H, W]`.
        if x.shape[1] == self.out_channels and isinstance(self.proj, nn.Identity):
            return x
        if isinstance(self.proj, nn.Conv2d) and x.shape[1] != self.in_channels:
            raise RuntimeError(f'InputAdapter expects {self.in_channels} channels, but got {x.shape[1]} channels. '
                               f'Please enable auto-detect or set --in-channels accordingly.')
        return self.proj(x)
...
def evaluate(model, loader, device, num_classes=2, tta=False):
    ...
    for data, labels, _ in loader:
        # `data` is `[B, T, 3, H, W]` when temporal clips are enabled, otherwise `[B, 3, H, W]`.
        data = data.to(device)
        ...
        if not tta:
            ...
            loss = ce_eval(logits, labels)
            probs = torch.softmax(logits, dim=1)
        # Model outputs `[B, num_classes]`; softmax keeps the same batch size for probabilities.
...
def evaluate_collect(model, loader, device, tta=False):
    """Collect labels and positive-class probabilities for threshold sweeping."""
    model.eval()
    all_labels=[]; all_scores=[]
    for data, labels, _ in loader:
        # Shapes mirror those used in `evaluate`, keeping batch-first semantics.
        data = data.to(device)
        if not tta:
            logits = model(data)
        else:
...
def sweep_threshold(y_true, y_score, step=0.001):
    # Sweep thresholds to find the maximum Youden's J statistic.
...
def mixup_data(x, y, alpha=0.2):
    ...
    mixed_x = lam * x + (1 - lam) * x[index, :]     # Keeps the original tensor shape (e.g., `[B, C, H, W]`).
    y_a, y_b = y, y[index]
...
def cutmix_data(x, y, alpha=0.2):
    ...
    # Spatial patch swap keeps batch/channel dims intact; output stays `[B, C, H, W]`.
...
def main():
    ...
    parser.add_argument('--temporal', type=int, default=0, help='0: single frame; 1: temporal')
    parser.add_argument('--T', type=int, default=9)
    parser.add_argument('--in-channels', type=int, default=1, help='Input channels (grayscale=1, RGB=3)')
    ...
    # Threshold sweep step (default 0.001; use 0.005/0.01 for faster scans)
    ...
    # Each dataset sample yields `[T, 3, img_size, img_size]` when `temporal=1`, otherwise `[3, img_size, img_size]`.
    ...
    # Probe one batch to determine the actual input channel count
    tmp_loader = DataLoader(train_dataset, batch_size=1, shuffle=False, num_workers=0)
    tmp_data, _, _ = next(iter(tmp_loader))
    # `tmp_data` retains the dataset layout; channel dimension sits at index 1 (`[1, C, H, W]` or `[1, T, 3, H, W]`).
    actual_in_ch = tmp_data.shape[1]
    if actual_in_ch != args.in_channels:
        print(f'[INFO] Detected input channels = {actual_in_ch}, override --in-channels {args.in_channels} -> {actual_in_ch}')
        args.in_channels = int(actual_in_ch)

    # Model: InputAdapter + TMS_CMAN
    ...
    # DataParallel wraps the adapter + backbone so the overall forward expects `[B, T, in_channels, H, W]`.
    ...
    if args.epochs != total_stage_epochs:
        print(f'[WARN] args.epochs({args.epochs}) != stage_plan total({total_stage_epochs}); training will follow stage_plan for {total_stage_epochs} epochs.')

    ema = ModelEMA(model, decay=0.999, device=device) if args.ema else None

    # Logging file paths (WRITE-EVERY-EPOCH)
    ...
    best_auc_epoch = -1
    no_improve = 0
    best_thr = 0.5  # Optimal threshold learned on the validation set (max Youden's J)
    ...
    def flush_metrics_to_files(records_list):
        ...
        try:
            ...
            print(f'[WARN] to_excel failed ({e}); CSV saved to: {csv_full}')
        else:
    ...
    # Apply the stage-specific unfreezing strategy
    ...
    # Optimizer / scheduler
        ...
        for e in range(stage_epochs):
            ...
            for data, labels, _ in pbar:
                # Loader delivers CPU tensors with batch-first layout; temporal clips are `[B, T, 3, H, W]`.
                data = data.to(device)
                labels = labels.to(device)
                ...
                if use_amp:
                    with torch.amp.autocast('cuda', enabled=True):
                        if use_aug and (do_mixup or do_cutmix):
                            if do_cutmix:
                                data_aug, y_a, y_b, lam = cutmix_data(data, labels, alpha=args.mix_alpha)
                            else:
                                data_aug, y_a, y_b, lam = mixup_data(data, labels, alpha=args.mix_alpha)
                            logits = model(data_aug)      # Outputs `[B, num_classes]`.
                            loss = mix_criterion(criterion, logits, y_a, y_b, lam)
                        else:
                            logits = model(data)          # Outputs `[B, num_classes]`.
            ...
            # Validation (compute standard metrics first, then sweep thresholds)
            ...
            best_thr = best_on_val['thr']  # Update the global best threshold
            ...
            # Log metrics (including threshold sweep results)
            ...
            # Save best checkpoint (still using AUC as the primary criterion)
    ...
    # Final evaluation (use EMA model, apply validation best_thr)
    eval_model = ema.ema if ema is not None else model
    ...
    # Test metrics derived from the learned best_thr
    ...
    # Record final test results (including both threshold variants)
    records.append({
        'epoch': current_epoch,
        'stage': 'final_test',
        'train_loss': np.nan,
